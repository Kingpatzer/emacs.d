...
(defmacro doom-log (format-string &rest args)
  "Log to *Messages* if `+my-debug-p' is on.
...
(defun doom--handle-load-error (e target path)
  (let* ((source (file-name-sans-extension target))
         (err (cond ((not (featurep 'core))
                    ...
                    ((file-in-directory-p source +my-core-dir)
                     (cons '+my-error +my-core-dir))
                    ((file-in-directory-p source +my-private-dir)
                     (cons '+my-private-error +my-private-dir))
                    ((cons '+my-module-error +my-emacs-dir)))))
    (signal (car err)
